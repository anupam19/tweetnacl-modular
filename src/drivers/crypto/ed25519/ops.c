...
/* Ed25519 signing */
int ed25519_sign(uint8_t *sig, unsigned long long *sig_len,
...
int ed25519_verify(uint8_t *msg, unsigned long long *msg_len,
                   const uint8_t *sm, size_t sm_len, const uint8_t *pk) {
    if (!msg || !msg_len || !sm || !pk) return -1;

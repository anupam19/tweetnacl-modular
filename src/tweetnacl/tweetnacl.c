...
/* UBSan suppression for functions with expected signed overflow */
...
sv add(gf p[4], gf q[4]) {
    gf a = {0}, b = {0}, c = {0}, d = {0}, t = {0}, e = {0}, f = {0}, g = {0}, h = {0};
...
NO_SANITIZE_UNDEFINED int crypto_sign_keypair(u8 *pk, u8 *sk) {
    u8 d[64];
    gf p[4];
    int i;
    int ret = 0;
...
static int unpackneg(gf r[4], const u8 p[32]) {
    gf t = {0}, chk = {0}, num = {0}, den = {0}, den2 = {0}, den4 = {0}, den6 = {0};
    set25519(r[2], gf1);
    unpack25519(r[1], p);
    S(num, r[1]);
    M(den, num, D);
